...
class OptimalOrderExecution:
    ...
    initial_X_distribution: Distribution[float]
    eta_sd: float
    rho: float
    ...
    def get_mdp(self, t: int) -> MarkovDecisionProcess[PriceAndShares, int]:
        ...
        class OptimalExecutionMDP(MarkovDecisionProcess[PriceAndShares, int]):
            def step(
                self, p_r: NonTerminal[PriceAndShares], sell: int
            ) -> SampledDistribution[Tuple[State[PriceAndShares], float]]:
                def sr_sampler_func(
                    p_r=p_r, sell=sell
                ) -> Tuple[State[PriceAndShares], float]:
                    p_s: PriceAndShares = PriceAndShares(
                        price=p_r.state.price, shares=sell
                    )
    ...
    def get_states_distribution(
        ...
        def states_sampler_func() -> NonTerminal[PriceAndShares]:
            ...
            for i in range(t):
                ...
                rem -= sell
                X = rho * X + self.initial_X_distribution.sample() - 0.25
            return NonTerminal(PriceAndShares(price=price, shares=rem, X=X))
